...
Dans un monde exigeant une grande mobilité des personnes et des biens, se déplacer est devenu un enjeu majeur pour tous et toutes. Cependant, ces déplacements viennent souvent à l’encontre de préoccupations environnementales grandissantes. En effet, les trajets en avion ou même en voiture sont extrêmement polluants : il est donc pertinent de se pencher sur une de leurs alternatives la plus verte, c’est-à-dire le train.

Notre application s’adresse à ce problème de mobilité, en proposant à ses utilisateurs une alternative à la SNCF. Grâce aux lignes créées avec cette application, nos clients pourront être connectés à la France entière par train, et rechercher leurs trajets en toute simplicité, n’ayant qu’à entrer une gare d’arrivée et de départ. Notre application permettra aussi une gestion facile et en toute simplicité des lignes et trajets proposés, afin de proposer un réseau ferroviaire adapté aux besoins de ses clients.

Afin d’accomplir cela, nous avons complété un dossier d’analyse qui nous permettra de cibler les fonctionnalités de cette application, ainsi que la structure de code nécessaire pour les mettre en place. Nous commencerons ainsi par un cahier des charges décomposant les différents buts de l’application et les réflexions et questionnements effectués par l’équipe autour de ceux-ci, en plus de l’organisation mise en place de notre équipe. Puis nous verrons les différents diagrammes qui nous ont permis de modéliser les besoins de notre application, que cela soit en terme d’architecture de code, de classes ou encore de structure de la base de données utilisées. Nous verrons ainsi les diagrammes d’architecture, d’activité, de Gantt, de séquences, de classe, de base de données et de cas d’utilisation.
...
allons donc préciser quelles sont ces contraintes. De plus, nous
...
demande d’habilitation ou par identifiant pour la
...
ponctuelle, pour des raisons développées dans l'explication de notre diagramme d'architecture. Enfin, cette fonctionnalité nous demande aussi de mettre en place un
...
durant ce projet. Il n'est pas obligatoire de développer le
...
Le diagramme ci-dessous permet de visualiser les futurs avancements dans le développement de l'application. Le but est de créer une première structure, en commençant par les classes, puis la base de données et la DAO, et enfin le service API. Une fois la structure réalisée et les différents liens entre la base de données, la DAO et l'utilisateur mis en place, le développement des fonctionnalités pourra commencer. Entre chaque session de développement, est organisé des sessions de tests, afin de créer ces derniers tout au long de la création de l'application. Les derniers jours seront dédiés à t'eventuels ajouts de fonctionnalités, ou de la correction de bugs. Cet emploi du temps pourra cependant évoluer en fonction de l'avancement des différentes phases de développement.
...
Le diagramme de cas d'utilisation ci-dessous présente l'ensemble des fonctionnalités offertes par l'application ainsi que les droits d'accès associés à chaque type d'utilisateur.
```{r}
#| cache: true
#| label: fig-usecase
#| fig-cap: "Diagramme de cas d'utilisation"
#| echo: false
#| out-width: "70%"

knitr::include_graphics("diagramme use case.jpg")
```
Quatre acteurs ont été distingués : l'utilisateur, qui représente toute personne non connectée pouvant créer un compte ou se connecter, le client, le collaborateur et l'administrateur. Ces quatre acteurs sont organisés de façon hiérarchique : le client hérite des actions disponibles pour l'utilisateur, le collaborateur hérite de celles du client, et l'administrateur hérite à son tour de celles du collaborateur. Cette hiérarchie permet de représenter simplement, au niveau du diagramme de cas d'utilisation, la progression des droits d'accès d'un rôle à l'autre ; elle n'implique pas pour autant que ces rôles soient représentés par des classes distinctes au niveau de la base de données, où un même compte reste stocké dans une seule table et où son rôle est simplement modifié par un attribut.

Le client peut notamment rechercher un trajet, ce qui inclut nécessairement la récupération des informations relatives à ce trajet. Cette récupération d'informations se décline ensuite en plusieurs extensions optionnelles : la récupération des tarifs, celle des places restantes, celle de la durée du trajet, ainsi que celle de la date et de l'heure de départ, qui permettent ensemble de visualiser la date de départ et la date d'arrivée. Le collaborateur dispose quant à lui des droits nécessaires pour créer, modifier et supprimer une ligne d'exploitation. Enfin, l'administrateur est seul habilité à gérer les comptes utilisateurs, ce qui recouvre notamment la modification de leur niveau d'habilitation.
...
à la recherche sont renvoyées sous forme de liste au format JSON. Enfin
ces gares sont affichées à l’utilisateur via l’interface de
l’application.
...
knitr::include_graphics("diagramme_seq_recherche_gare1.png")
```
...
knitr::include_graphics("diagramme_seq_creation_trajet1.png")
```


# VII) Diagramme de base de données

Le schéma suivant présente la structure de la base de données conçue pour l'application. Celle ci a été pensée pour répondre aux quatre fonctionnalités principales du cahier des charges : la gestion des comptes, la gestion des lignes d'exploitation, la planification des trajets et la gestion des réservations.
```{r}
#| cache: true
#| label: fig-bdd
#| fig-cap: "Diagramme de base de donneés"
#| echo: false
#| out-width: "60%"

knitr::include_graphics("diagramme de base de données.png")
```
La table utilisateurs centralise l'ensemble des comptes de l'application, qu'il s'agisse d'un client, d'un collaborateur ou d'un administrateur. Plutôt que de créer une table par type de compte, nous avons choisi de les regrouper au sein d'une même entité et de les distinguer par l'attribut role. Ce choix se justifie par le fait qu'un compte peut changer de niveau d'habilitation au cours de son existence, un administrateur pouvant promouvoir ou rétrograder un utilisateur, ce qui aurait été plus contraignant à gérer si chaque rôle correspondait à une table distincte.

La table gares constitue une table de référence : elle sert à stocker localement les gares déjà validées auprès de l'API SNCF, afin d'éviter de solliciter systématiquement le service externe à chaque utilisation. Chaque ligne d'exploitation, stockée dans la table lignes, référence une gare de départ et une gare de terminus grâce à deux clés étrangères pointant toutes deux vers la table gares (gare_depart_id et gare_arrivee_id), ce qui explique le double lien visible sur le schéma. Chaque ligne est par ailleurs rattachée au compte collaborateur qui l'a créée, par l'intermédiaire de la clé id_collaborateur.

Une fois une ligne créée, un collaborateur peut y planifier un ou plusieurs trajets, stockés dans la table trajets. Chaque trajet reprend l'identifiant de sa ligne d'exploitation ainsi que les horaires de départ et d'arrivée, et conserve le temps de trajet calculé automatiquement à partir de l'API SNCF. Enfin, la table reservations enregistre les réservations effectuées sur un trajet donné : elle conserve le nombre de places restantes au moment de la réservation, ainsi que le prix et le tarif selon la classe de voyage appliqués à cette réservation.
...
consiste à définir les classes représentant les objets manipulés
par l'application.
...
tels qu'un utilisateur, une réservation ou un trajet. \
En premier lieu, la classe Utilisateur représente un utilisateur de l’application. Un
utilisateur est défini par son identifiant unique (`id`), et possède un
nom d’utilisateur (`nom_utilisateur`), un mot de passe (`mdp`) et son rôle. L’utilisateur peut
avoir le statut (`role`) de client, collaborateur ou administrateur sur
l’application, modélisés par les classes filles correspondantes. Cela
permettra d’implémenter des fonctionnalités différentes pour les
utilisateurs en fonction de leur niveau d’habilitation. \
La classe Reservation, associée à la classe client, modélise les réservations
...
comptabilité. \
La classe Reservation est associée à une classe Trajet
...
trajet. \
Une classe Train est également associée à la classe Trajet. Elle
...
capacités variant selon le type de train. \
La classe Ligne, associée à
...
Rennes et Rennes -\> Paris sont deux lignes différentes. \
La classe Gare
...
Service (diagramme ci-dessus). \
Les classes Service liées aux
...
cadre de `recherche_compte()`. \
La classe ClientService contient la
...
correspondent aux actions spécifiques au rôle de client. \
La classe CollaborateurService regroupe les méthodes `creer_ligne()` et
`creer_trajet()` qui permettent à un collaborateur d’ajouter une
nouvelle ligne ferroviaire ou un nouveau trajet sur le réseau. \
La classe AdministrateurService contient les méthodes `gerer_habilitation()`, qui
...
de retrouver un compte utilisateur (par id ou nom d’utilisateur). \
Par ailleurs, d'autres classes Service permettent de gérer les objets
...
TrajetService.creer()). Enfin, ReservationService dépend de TrajetService
